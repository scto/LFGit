...
public abstract class BasicAbstractActivity extends AppCompatActivity {
  ...
  public void onRequestPermissionsResult(
    ...
    if (requestCode
        == PERMISSIONS_REQUEST) { // If request is cancelled, the result arrays are empty.
  ...
  public void showOptionsDialog(
      int title, int optionsResource, final onOptionClicked[] option_listeners) {
    CharSequence[] options_values = getResources().getStringArray(optionsResource);
    AlertDialog.Builder builder = new AlertDialog.Builder(this);
    builder.setTitle(title);
    builder
        .setItems(options_values, (dialog, which) -> option_listeners[which].onClicked())
        .create()
        .show();
  }
